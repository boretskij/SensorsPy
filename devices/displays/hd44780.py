...
# Modified by https://github.com/zagaria
...
class HD44780:
    ...
    def __init__(self, addr, port, numlines=2, clear=True):
        ...
        _displayfunction = HD44780._LCD_4BITMODE | \
            HD44780._LCD_2LINE | HD44780._LCD_5x8DOTS
        self._command(HD44780._LCD_FUNCTIONSET | _displayfunction)

        # turn the display on with no cursor or blinking default
        self._displaycontrol = HD44780._LCD_DISPLAYON | \
            HD44780._LCD_CURSOROFF | HD44780._LCD_BLINKOFF
        self._command(HD44780._LCD_DISPLAYCONTROL | self._displaycontrol)

        # Initialize to default text direction (for roman languages)
        self._displaymode = HD44780._LCD_ENTRYLEFT | \
            HD44780._LCD_ENTRYSHIFTDECREMENT;
        # set the entry mode
        self._command(HD44780._LCD_ENTRYMODESET | self._displaymode);
    ...
    def clear(self):
        ...
        self._command(HD44780._LCD_CLEARDISPLAY)
        time.sleep(2) # this command takes a long time!

    def home(self):
        """set cursor position to zero"""
        self._command(HD44780._LCD_RETURNHOME)
        time.sleep(2) # this command takes a long time!
    ...
    def setCursor(self, col, row):
        ...
        self._command(HD44780._LCD_SETDDRAMADDR | \
            (col + row_offsets[row]))
    ...
    def createChar(self, location, charmap):
        ...
        self._command(HD44780._LCD_SETCGRAMADDR | (location << 3))
        for i in range(0,8):
          self._send(charmap[i], HD44780._Rs)

    def noBacklight(self):
        """Turn off the (optional) backlight"""
        self._backlightval = HD44780._LCD_NOBACKLIGHT
        self._expanderWrite(0);

    def backlight(self):
        """Turn on the (optional) backlight"""
        self._backlightval = HD44780._LCD_BACKLIGHT
        self._expanderWrite(0);
    ...
    def _pulseEnable(self, data):
        self._expanderWrite(data | HD44780._En) # En high
        time.sleep(0.000001) # enable pulse must be >450ns

        self._expanderWrite(data | ~HD44780._En) # En low
